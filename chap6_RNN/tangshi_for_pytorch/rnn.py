...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim):
        ...
        self.rnn_lstm = nn.LSTM(embedding_dim,lstm_hidden_dim,2,batch_first=True)
        ##########################################
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        h1 = torch.zeros(2, 1, self.lstm_dim).to(sentence.device)
        c1 = torch.zeros(2, 1, self.lstm_dim).to(sentence.device)
        output, _ = self.rnn_lstm(batch_input, (h1, c1))
        ################################################
